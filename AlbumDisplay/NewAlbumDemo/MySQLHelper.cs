...
namespace NewAlbumDemo
{
    public class MySQLHelper
    {
        ...
        public MySqlCommand CreateCmd(ModelAlbum model,EnumStatus status)
        {
            String SQL = "";
            switch (status)
            {
                case EnumStatus.Insert:
                    SQL = "insert album (photocaption,photoURL,photoDescription,Lables)values('" + model.photoCaption + "','" + model.photoURL.ToString() + "','" + model.photoDescription + "','" + String.Join(',', model.labels.Select(x=>x.value).ToList()) + "')";
                    break;
                case EnumStatus.Update:
                    SQL = String.Format("update album set photocaption='{0}',photoURL='{1}',photoDescription='{2}',Lables='{3}' where photoID='{4}'",model.photoCaption,model.photoURL,model.photoDescription,String.Join(',',model.labels.Select(x=>x.value).ToList()),model.photoID);
                    break;
                case EnumStatus.Delete:
                    SQL = String.Format("delete from album where photoID={0}",model.photoID);
                    break;
                case EnumStatus.Query:
        ...
        public void getInsert(MySqlCommand mySqlCommand)
        {
            try
            {
                var successCount = mySqlCommand.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
